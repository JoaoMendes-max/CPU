...
\begin{itemize}
    ...
    \item The current bus implementation is single-cycle when selected\footnote{The current validated board-operating point is 50\,MHz after hardware bring-up exposed functional hazards at higher clocks under the current implementation/debug setup.} (\texttt{rdy = sel} for each block) and uses a read-data mux; this is primarily a microarchitectural choice rather than a special constraint trick.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Clock definition for the system clock (50MHz operational target on pin K17).
...
create_clock -name sys_clk -period 20.000 -waveform {0.000 10.000} [get_ports i_clk]
\end{lstlisting}

\texttt{create\_clock} informs the timing engine:
\begin{itemize}
    \item Period: 20ns (50MHz).
    \item Rising edge at 0ns, falling edge at 10ns.
...
\subsection{Clock Frequency: 50MHz Operational Target}
Although post-implementation timing reported WNS $> 0$ at higher targets, hardware validation used 50\,MHz because functional hazards were observed above this point in the current build/debug configuration. Reasons:
\begin{enumerate}
...
Future optimizations (Pblocks, manual placement hints, and additional hazard closure) may recover higher hardware frequencies.
...
\begin{itemize}
    ...
    \item Bare-metal programs execute at a 50\,MHz operating point chosen for robust behavior under debug instrumentation.
\end{itemize}
